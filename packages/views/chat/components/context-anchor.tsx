...
import type { ContextAnchor } from "@nimbus/core/chat";
...
import { inboxListOptions } from "@nimbus/core/inbox/queries";
...
import { useWorkspacePaths } from "@nimbus/core/paths";
...
export function ContextAnchorButton() {
  ...
  const tooltipText = isDisabled
    ? "Nothing to share with Nimbus on this page"
    : focusMode && candidate
      ? candidate.type === "issue"
        ? `Nimbus knows you're viewing ${candidate.label} · Click to turn off`
        : `Nimbus knows you're viewing project "${candidate.label}" · Click to turn off`
      : "Let Nimbus know what you're viewing";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            disabled={isDisabled}
            aria-label={
              focusMode ? "Stop sharing current page" : "Share current page with Nimbus"
            }
...
export function ContextAnchorCard() {
  ...
  const tooltipText =
    candidate.type === "issue"
      ? `Nimbus knows you're viewing ${candidate.label}${candidate.subtitle ? ` — ${candidate.subtitle}` : ""}`
      : `Nimbus knows you're viewing project "${candidate.label}"`;
