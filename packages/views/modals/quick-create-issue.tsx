...
import { DialogTitle } from "@nimbus/ui/components/ui/dialog";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { useCurrentWorkspace } from "@nimbus/core/paths";
import { agentListOptions } from "@nimbus/core/workspace/queries";
import { useQuickCreateStore } from "@nimbus/core/issues/stores/quick-create-store";
...
} from "@nimbus/core/runtimes";
import { useFileUpload } from "@nimbus/core/hooks/use-file-upload";
import { formatShortcut, modKey, enterKey } from "@nimbus/core/platform";
import type { Agent } from "@nimbus/core/types";
import { ActorAvatar } from "../common/actor-avatar";
import { canAssignAgent } from "../issues/components/pickers/assignee-picker";
import { useAuthStore } from "@nimbus/core/auth";
import { memberListOptions } from "@nimbus/core/workspace/queries";
import {
  ContentEditor,
...
export function AgentCreatePanel({
  ...
  // bundled nimbus CLI to be ≥ MIN_QUICK_CREATE_CLI_VERSION; older
        ...
        {selectedAgent && versionBlocked && (
          <div className="mx-5 mb-2 shrink-0 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
            {versionCheck.state === "missing"
              ? `This agent's daemon doesn't report a CLI version. Create with agent needs nimbus CLI ≥ ${versionCheck.min}. Upgrade the daemon and reconnect, or switch to manual create.`
