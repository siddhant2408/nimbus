...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Nimbus' title", () => {
    render(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to nimbus/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("nimbus_token", "existing-jwt");
