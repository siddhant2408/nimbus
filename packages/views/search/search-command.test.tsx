...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.nimbus/${p}`),
  mockWorkspaces: {
...
vi.mock("@nimbus/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.nimbus/${p}`);
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.nimbus//ws-test/issues/issue-1");
