import type { QuestionnaireAnswers } from "@nimbus/core/onboarding";
import type {
  ImportStarterContentPayload,
  ImportStarterIssuePayload,
} from "@nimbus/core/api";
...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how Nimbus works →](https://nimbus.ai/docs/how-nimbus-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in Nimbus**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus. They're **exploring** what Nimbus can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to Nimbus — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Nimbus.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in Nimbus are triggered when an issue has:`,
        ``,
        ...
        ``,
        `[Learn about assigning issues →](https://nimbus.ai/docs/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about chat →](https://nimbus.ai/docs/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about workspaces →](https://nimbus.ai/docs/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `Nimbus works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
        ...
        ``,
        `[Learn about members and roles →](https://nimbus.ai/docs/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[Learn about creating agents →](https://nimbus.ai/docs/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[Learn about tasks →](https://nimbus.ai/docs/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about the inbox →](https://nimbus.ai/docs/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about autopilots →](https://nimbus.ai/docs/autopilots)`,
      ].join("\n"),
    },
  ];
...
export function buildSelfServeSubIssues(
  ...
  // Without a runtime + an agent, nothing else in Nimbus works. These two
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/nimbus-ai/nimbus/releases/latest](https://github.com/nimbus-ai/nimbus/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        `   \`\`\``,
        `   curl -fsSL https://raw.githubusercontent.com/nimbus-ai/nimbus/main/scripts/install.sh | bash`,
        `   \`\`\``,
        ...
        ``,
        `[Learn about runtimes →](https://nimbus.ai/docs/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in Nimbus. Work through them at your own pace.",
      icon: "👋",
    },
