 ...
 *   nimbus-desktop-{version}-mac-{arch}.{dmg|zip}
 *   nimbus-desktop-{version}-windows-{arch}.exe
 *   nimbus-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^nimbus-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
